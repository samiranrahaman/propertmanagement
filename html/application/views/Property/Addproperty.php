<div class="content-wrapper" ng-controller="addpropertycontroller">
			...
			<div class="row row-centered">
	           ...
	           <form  id="addproperty" name="addproperty" ng-submit="addpropertyform()" novalidate >
				...
				<!--<div class="form-group has-feedback" ng-class="{ 'has-error' : addproperty.property_name.$invalid && !addproperty.property_name.$pristine }">
					...
					<input type="text" class="form-control" placeholder="Property Name" id="property_name" name="property_name" ng-model="property_name"  value="{{property_type}}" required> 
				...
				<div class="form-group"  ng-class="{ 'has-error' : addproperty.property_type.$invalid && !addproperty.property_type.$pristine }">
						...
						<!-- <select class="form-control"  id="property_type" name="property_type"  required ng-model="property_type" ng-change="getbuildingval()"
							ng-options="opt.id as opt.title for opt in propertylist">
						</select>-->
						<select class="form-control"  id="property_type" name="property_type"  required ng-model="property_type" ng-change="getbuildingval()"
							ng-options="opt.title for opt in buildinglist">
						</select>
 ...
 app.controller('addpropertycontroller', function($scope,$http){
  ...
  $http({
	...
	$scope.getbuildingval=function()
	{
		 /* alert($scope.property_type.id);
		 alert($scope.property_type.title);
		  */
		 $http({
		 method:'post',
		 url:'<?php echo base_url();?>index.php/property/get_building_floor_details',
		 headers: {'Content-Type': 'application/x-www-form-urlencoded'},
		 data :JSON.stringify({building_id:$scope.property_type.id})
	     }).success(function(data){
		 console.log(data);
	 ...
	 $scope.addpropertyform=function()
	 {
		//alert($scope.property_type.id);
		// alert($scope.property_type.title);
		 // alert($scope.property_floor);
		 // alert($scope.flat_no);
		  
		var property_name=$scope.property_type.title+'/'+$scope.property_floor+'/'+$scope.flat_no;
		// alert(property_name); 
		...
		$http({
		 ...
		 property_type_id:$scope.property_type.id,
		 building_name:$scope.property_type.title,
