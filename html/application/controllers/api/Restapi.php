...
class Restapi extends REST_Controller 
{
   public function __construct()
	...
	public function change_users_type_get()
    {
		$user_id = $this->get('user_id');
		$user_type = $this->get('user_type');
		
		$result=$this->User_model->change_users_type($user_id,$user_type);
		if($result === FALSE)
        {
            $this->response(array(
                    'status' => FALSE,
	...
	function user_reg_get()
    {
		 ...
		 if($building_id >0)
		 {
			 $flat_no = $this->get('flat_no');
			 if($flat_no<=0)
			 {
				 $flat_no=0;
			 }
			 
			 $floor_no = $this->get('floor_no');
			 if($floor_no<=0)
			 {
				 $floor_no=0;
			 }
			...
			$firstname = $this->get('firstname');
             ...
             $result = $this->User_model->post_add_user_tenant_api($firstname,$lastname,$phone1,$email,$user_type,$username,$password,$device_token,$building_id,$flat_no,$floor_no); 
	...
	public function add_owner_property_get()
	{
		     ...
		     $result=$this->User_model->add_owner_property($user_id,$building_id,$flat_no,$floor_no);	
		    if($result === FALSE)
				{
					$this->response(array(
							'status' => FALSE,
	...
	/* public function edit_owner_property_get()
	{
		        $property_id = $this->get('property_id');
			    $user_id = $this->get('user_id');
			    $building_id = $this->get('building_id');
		     ...
		     $result=$this->User_model->edit_owner_property($user_id,$building_id,$flat_no,$floor_no,$property_id);	
		    if($result === FALSE)
				{
					$this->response(array(
							'status' => FALSE,
	...
	/* public function edit_tenant_property_get()
	{
		        $property_id = $this->get('property_id');
			    $user_id = $this->get('user_id');
			    $building_id = $this->get('building_id');
		     ...
		     $result=$this->User_model->edit_tenant_property($user_id,$building_id,$flat_no,$floor_no,$property_id);	
		    if($result === FALSE)
				{
					$this->response(array(
							'status' => FALSE,
	...
	public function edit_property_get()
	{
		     ...
		     $result=$this->User_model->edit_property($building_id,$flat_no,$floor_no,$property_id);	
		    if($result === FALSE)
				{
					$this->response(array(
							'status' => FALSE,
	...
	public function tenant_property_details_get()
	{
               $user_id = $this->get('user_id');
			 
		     $result=$this->Property_model->get_property_details($user_id);	
		     if(!empty($result))
				{
					$this->response(array(
							'status' => TRUE,
	...
	public function owner_property_details_get()
	{
               $user_id = $this->get('user_id');
			 
		     $result=$this->Property_model->get_owner_property_details($user_id);	
		     if(!empty($result))
				{
					$this->response(array(
							'status' => TRUE,
    ...
    {
		 $security_id = $this->get('security_id');
         $result = $this->Visitorsmanage_model->visitor_request_list($security_id);
    ...
    {
		$security_id = $this->get('security_id'); 
         $result = $this->Visitorsmanage_model->get_visitors_list($security_id);
	...
	function sent_global_notification_get()
    {
		 ...
		 $security_id = $this->get('security_id');
         $result = $this->Security_model->post_global_notification($subject,$message,$security_id);
	...
	function property_building_list_get()
    {
		 $result = $this->Property_model->get_property_type();
         if(!empty($result))
				{
					$this->response(array(
							'status' => TRUE,
	...
	function property_building_floor_list_get()
    {
		 $building_id = $this->get('building_id'); 
		 $result = $this->Property_model->get_building_floor_details($building_id);
         if(!empty($result))
				{
					$this->response(array(
							'status' => TRUE,
