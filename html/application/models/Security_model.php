...
class Security_model extends CI_Model
{
	...
	public function post_global_notification($subject,$message,$security_id)
	{
		
		             /* $sql1="select device_token ,id from  `tent_userdata` where status=1 and user_type!=16 "; */
		 $sql1="SELECT 	* FROM  `tent_userdata` a,
			ten_propert_list b
			where a.id=".$security_id."
		...
		if(!empty($query_res1)){
			 $property_type_id=$query1->row()->property_type;	 
					 $sql1="select a.device_token ,a.id from 
					 `tent_userdata`a ,ten_propert_list b
					 where a.property_id=b.id 
					 and b.property_type=$property_type_id 
					 and  a.status=1 and a.user_type!=16 ";
